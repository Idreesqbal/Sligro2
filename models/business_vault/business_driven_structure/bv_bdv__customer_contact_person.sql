...
WITH rv_sat__customer_contact_person AS (
    SELECT *
    FROM {{ ref('rv_sat__data_hub_customer_contact_person') }}
...
unique_key_creation AS (
    SELECT
        *,
        ...
        concat(customerid, '||', registrationdate, '||', contactpersonid) AS contact_person_bk
    FROM rv_sat__customer_contact_person
),

{# Hashing the BKs #}
create_hashed_columns AS (
    SELECT
        * EXCEPT (customer_hk, contact_person_hk),
        upper(to_hex(sha256(customer_bk))) AS customer_hk,
        upper(to_hex(sha256(contact_person_bk))) AS contact_person_hk
    ...
    SELECT
        * EXCEPT (end_datetime),
        lag(load_datetime) OVER (
            PARTITION BY customer_hk, contact_person_hk
