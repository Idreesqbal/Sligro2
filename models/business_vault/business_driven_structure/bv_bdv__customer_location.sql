...
WITH rv_sat__customer_location AS (
    SELECT *
    FROM {{ ref('rv_sat__data_hub_customer_location') }}
...
unique_key_creation AS (
    SELECT
        *,
        ...
        concat(customerid, '||', registrationdate, '||', addressid) AS customer_address_bk
    FROM rv_sat__customer_location
),

{# Hashing the BKs #}
create_hashed_columns AS (
    SELECT
        * EXCEPT (customer_hk, customer_address_hk),
        upper(to_hex(sha256(customer_bk))) AS customer_hk,
        upper(to_hex(sha256(customer_address_bk))) AS customer_address_hk
    ...
    SELECT
        * EXCEPT (end_datetime),
        lag(load_datetime) OVER (
            PARTITION BY customer_hk, customer_address_hk
