{#
    This table contains the full list of customer IDs and is used for the new customer HUB in the business vault.
    For entities that need a link with customer in the business vault, this is where you need to get the registration_date
    from to create the new customer BK.
#}
...
WITH rv_sat__customer_customer AS (
    SELECT *
    FROM {{ ref('rv_sat__data_hub_customer_customer') }}
...
unique_key_creation AS (
    ...
    FROM rv_sat__customer_customer
),
