...
WITH rv_sat__customer_characteristics AS (
    SELECT *
    FROM {{ ref('rv_sat__data_hub_customer_characteristics') }}
...
unique_key_creation AS (
    ...
    FROM rv_sat__customer_characteristics
),
    ...
    SELECT
        * EXCEPT (end_datetime),
        lag(load_datetime) OVER (
            PARTITION BY customer_hk, characteristictype, characteristiccode
