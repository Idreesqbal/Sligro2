...
WITH
record_to_insert AS (
    -- Filter out rv_sat__supplier_general_info for supplier that acts as carrier and preserved the historical data.
    SELECT
        link_carrier_supplier.carrier_hk,
        sat_supplier_general.*
    FROM {{ ref("rv_hub__supplier") }} AS hub_supplier
    INNER JOIN
        {{ ref("rv_sat__supplier_general_info") }} AS sat_supplier_general
...
-- The record source is replaced into rv_sat__supplier_general_info instead of AS400 table
-- Load data where records do not exist or if end_datetime is updated
SELECT * EXCEPT (supplier_hk) REPLACE ('rv_sat__supplier_general_info' AS record_source) FROM record_to_insert
{% if is_incremental() %}
    WHERE NOT EXISTS (
        SELECT 1 AS exist
