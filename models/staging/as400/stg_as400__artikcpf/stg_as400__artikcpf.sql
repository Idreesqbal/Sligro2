{%- set yaml_metadata -%}
source_model:
    'base_as400__artikcpf'
derived_columns:
    RECORD_SOURCE: "!AS400__artikcpf"
    LOAD_DATETIME: "{{ fivetran_last_touched() }}"
    EFFECTIVE_FROM: "_fivetran_start"
    START_DATETIME: "_fivetran_start"
    END_DATETIME: "{{ fivetran_end() }}"
    ARTICLE_BK:
        - "TRIM(CAST(ARNRC1 AS STRING))"
    ARTICLE_GROUP_NK:
        - "TRIM(CAST(ARGRC1 AS STRING))"
    SITE_NK:
        - "!{{ var("cdc_sitecode") }}" # HR.001
    COLLECTION_AREA_NK:
        - "!{{ var("cdc_sitecode") }}" # HR.001
        - "TRIM(CAST(DCKDC1 AS STRING))"
        - "TRIM(CAST(AREAC1 AS STRING))"
    STOCK_LOCATION_BK:
        - "!{{ var("cdc_sitecode") }}" # HR.001
        - "TRIM(CAST(DCKDC1 AS STRING))"
        - "TRIM(CAST(AREAC1 AS STRING))"
        - "LPAD(TRIM(CAST(PGNRC1 AS STRING)), 3, '0')"
        - "LPAD(TRIM(CAST(PVNRC2 AS STRING)), 2, '0')"
        - "TRIM(CAST(PLNRC2 AS STRING))"
        - "LPAD(TRIM(CAST(PPNRC2 AS STRING)), 2, '0')"
    SUPPLIER_NK:
        - "TRIM(CAST(LVNRC1 AS STRING))"
    PGNRC1:
        - "LPAD(TRIM(CAST(PGNRC1 AS STRING)), 3, '0')"
    PVNRC2:
        - "LPAD(TRIM(CAST(PVNRC2 AS STRING)), 2, '0')"
    PPNRC2:
        - "LPAD(TRIM(CAST(PPNRC2 AS STRING)), 2, '0')"
hashed_columns:
    ARTICLE_HK:
        - "ARTICLE_BK"
    ARTICLE_GROUP_HK:
        - "ARTICLE_GROUP_NK"
    SITE_HK:
        - "SITE_NK"
    COLLECTION_AREA_HK:
        - "COLLECTION_AREA_NK"
    STOCK_LOCATION_HK:
        - "STOCK_LOCATION_BK"
    SUPPLIER_HK:
        - "SUPPLIER_NK"
    ARTICLE_LOCATION_SUPPLIER_HK:
        - "ARTICLE_BK"
        - "ARTICLE_GROUP_NK"
        - "SITE_NK"
        - "COLLECTION_AREA_NK"
        - "STOCK_LOCATION_BK"
        - "SUPPLIER_NK"
    SITE_COLLECTION_AREA_STOCK_LOCATION_HK:
        - SITE_NK
        - COLLECTION_AREA_NK
        - STOCK_LOCATION_BK
    ARTICLE_HASHDIFF:
        is_hashdiff: true
        columns:
            - "ARTICLE_BK"
            - "S013C1"
            - "VPOMC1"
            - "IVAAC1"
            - "DCAAC2"
            - "ODAAC1"
            - "PTAAC1"
            - "PLAAC1"
            - "ASKDC1"
            - "AROMC1"
            - "INPRC1"
            - "BSDDC1"
            - "BTKDC1"
            - "GANGC1"
            - "ANDVC1"
            - "VRKLC1"
            - "ETIKC1"
            - "GEWIC1"
            - "VOLMC1"
            - "OGKDC1"
            - "APPRC1"
            - "XCPRC1"
            - "VRKDC1"
            - "DPKDC1"
            - "HAKDC1"
            - "BOKDC1"
            - "MTGTC1"
            - "KLOMC1"
            - "AS01C1"
            - "AS02C1"
            - "AS03C1"
            - "AS04C1"
            - "AS05C1"
            - "AS06C1"
            - "AS07C1"
            - "AS08C1"
            - "AS09C1"
            - "AS10C1"
            - "AS11C1"
            - "AS12C1"
            - "AS13C1"
            - "AS14C1"
            - "AS15C1"
            - "AS16C1"
            - "AS17C1"
            - "AS18C1"
            - "AS19C1"
            - "AS20C1"
            - "AS21C1"
            - "AS22C1"
            - "AS23C1"
            - "AS24C1"
            - "AS25C1"
            - "AS26C1"
            - "AS27C1"
            - "AS28C1"
            - "AS29C1"
            - "AS30C1"
            - "ZBPRC1"
            - "CNPRC1"
            - "EMPRC1"
            - "MVAAC1"
            - "THTDC1"
            - "AARNC1"
            - "BNAAC1"
            - "ZBDTC1"
            - "ZNPRC1"
            - "ZVPRC1"
            - "INDTC1"
            - "ITPRC1"
            - "IVPRC1"
            - "NCPRC1"
            - "NBPRC1"
            - "ALCDC1"
            - "AWCDC1"
            - "CLKDC1"
            - "MNAAC1"
            - "VVDTC1"
            - "ED1BC1"
            - "ED2BC1"
            - "NVDTC1"
            - "KDOPC1"
            - "KRKDC1"
            - "ZP01C1"
            - "CP01C1"
            - "ZN01C1"
            - "CN01C1"
            - "TMKDC1"
            - "AS31C1"
            - "AS32C1"
            - "AS33C1"
            - "AS34C1"
            - "AS35C1"
            - "SLHOC1"
            - "SLSUC1"
            - "XCAAC1"
            - "SEVNC1"
            - "SLPRC1"
            - "DPPRC1"
            - "AS36C1"
            - "AS37C1"
            - "AS38C1"
            - "AS39C1"
            - "AS40C1"
            - "AS41C1"
            - "AS42C1"
            - "AS43C1"
            - "AS44C1"
            - "AS45C1"
            - "AS46C1"
            - "AS47C1"
            - "AS48C1"
            - "AS49C1"
            - "AS50C1"
            - "AS51C1"
            - "AS52C1"
            - "AS53C1"
            - "AS54C1"
            - "AS55C1"
            - "AS56C1"
            - "AS57C1"
            - "AS58C1"
            - "AS59C1"
            - "AS60C1"
            - "AS61C1"
            - "AS62C1"
            - "AS63C1"
            - "AS64C1"
            - "AS65C1"
            - "AS66C1"
            - "AS67C1"
            - "AS68C1"
            - "AS69C1"
            - "AS70C1"
            - "AS71C1"
            - "AS72C1"
            - "AS73C1"
            - "AS74C1"
            - "AS75C1"
            - "AS76C1"
            - "AS77C1"
            - "AS78C1"
            - "AS79C1"
            - "AS80C1"
            - "DEAFC1"
            - "PLURC1"
            - "ZK01C1"
            - "ZK02C1"
            - "EA01C1"
            - "EA02C1"
            - "INBAC1"
            - "WIDTC1"
            - "WZDTC1"
            - "IADEC1"
            - "YCPRC1"
            - "YNPRC1"
            - "YCAAC1"
            - "KOPRC1"
            - "KVPRC1"
            - "NKPRC1"
            - "ZP02C1"
            - "ZP03C1"
            - "ZP04C1"
            - "ZP05C1"
            - "ZP06C1"
            - "ZP07C1"
            - "ZP08C1"
            - "ZP09C1"
            - "ZP10C1"
            - "ZP11C1"
            - "ZP12C1"
            - "ZN02C1"
            - "ZN03C1"
            - "ZN04C1"
            - "ZN05C1"
            - "ZN06C1"
            - "ZN07C1"
            - "ZN08C1"
            - "ZN09C1"
            - "WYNWC1"
            - "SLPSC1"
            - "PRARC1"
            - "MTE1C1"
            - "MTE2C1"
            - "IMKDC1"
            - "MGGWC1"
            - "SPAAC1"
            - "SBAAC1"
            - "MGTXC1"
            - "OMOMC1"
            - "ACRFC1"
            - "ILITC1"
            - "ALCPC1"
            - "VEKDC1"
            - "CBNRC1"
            - "BD01C1"
            - "BD02C1"
            - "BD03C1"
            - "BD04C1"
            - "BD05C1"
            - "AKN2C1"
            - "GR01C2"
            - "GR03C1"
            - "GR04C2"
            - "_fivetran_start"
            - "_fivetran_end"
    STOCK_LOCATION_HASHDIFF:
        is_hashdiff: true
        columns:
            - "STOCK_LOCATION_BK"
            - "SITE_NK"
            - "DCKDC1"
            - "AREAC1"
            - "PGNRC1"
            - "PVNRC2"
            - "PLNRC2"
            - "PPNRC2"
            - "VPOMC1"
            - "_fivetran_start"
            - "_fivetran_end"
{%- endset -%}

{% set metadata_dict = fromyaml(yaml_metadata) %}

{{ automate_dv.stage(
      include_source_columns=true,
      source_model=metadata_dict['source_model'],
      derived_columns=metadata_dict['derived_columns'],
      null_columns=none,
      hashed_columns=metadata_dict['hashed_columns'],
      ranked_columns=none
  ) }}
