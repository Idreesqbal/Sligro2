...
final AS (
    SELECT
        bv_line.purchase_order_line_bk AS pk_purchase_order_line_code,
        ...
        ROUND(
            CAST(
                bv_line.inprbe + SAFE_MULTIPLY(bv_line.inprbe, {{ vat_rate('bv_line.btkdbe') }}) AS BIGNUMERIC
            ),
            2
        ) AS article_unit_price_including_vat,
    ...
    LEFT JOIN hub_article ON bv_lnk_purchase_order_article.article_hk = hub_article.article_hk
