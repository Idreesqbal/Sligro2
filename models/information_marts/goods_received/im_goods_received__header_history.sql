...
first_record AS (
    SELECT
        ...
        goods_received_hk
    FROM {{ ref("bv_bdv__goods_received") }}
    QUALIFY ROW_NUMBER() OVER (PARTITION BY goods_received_hk ORDER BY godtba ASC) = 1
),
...
final AS (
    SELECT
        CONCAT(
            hub_goods_received.goods_received_bk,
            "||",
            header.start_datetime,
            "||",
            COALESCE(header.end_datetime, {{ var('end_date') }})
        )
            AS pk_goods_received_history,
        hub_goods_received.goods_received_bk AS fk_goods_received_code,
        header.ionrba AS goods_received_code,
        ...
        header.znkdba AS communication_method,
        header.znkdba_description AS communication_method_description,
        header.stkdba AS goods_received_delivery_status,
        header.stkdba_description AS goods_received_delivery_status_description,
        header.lvdtba AS current_planned_date,
        header.godtba AS received_date,
        first_record.lvdtba AS original_planned_date,
        date_count.planned_date_change_count,
        header.start_datetime AS valid_from,
        header.end_datetime AS valid_to,
        header.load_datetime
    ...
    INNER JOIN goods_received_header AS header
        ON lnk.goods_received_hk = header.goods_received_hk
